...
use crate::kinode::process::file_transfer_template::{Address as WitAddress, Request as TransferRequest, Response as TransferResponse, WorkerRequest, DownloadRequest, ProgressRequest, FileInfo, InitializeRequest};
...
wit_bindgen::generate!({
    path: "target/wit",
    world: "file-transfer-template-test-diligence-dot-os-v0",
...
fn test_list_files(address: &Address) -> anyhow::Result<Vec<FileInfo>> {
    ...
    if response.is_request() { fail!("file_transfer_template_test"); };
    let TransferResponse::ListFiles(files) = response.body().try_into()? else {
        fail!("file_transfer_template_test");
    };
    Ok(files)
}
...
fn handle_message (our: &Address) -> anyhow::Result<()> {
    ...
    print_to_terminal(0, "file_transfer_template_test: a");
    ...
    let our_ft_address = Address {
        node: our.node.clone(),
        process: ProcessId::new(Some("file_transfer_template"), "file_transfer_template", "diligence.os"),
    };
    ...
    // Send
    print_to_terminal(0, "file_transfer_template_test: b");
    ...
    print_to_terminal(0, "file_transfer_template_test: c");
    let response = Request::new()
...
fn init(our: Address) {
    ...
    loop {
        match handle_message(&our) {
            Ok(()) => {},
            Err(e) => {
                print_to_terminal(0, format!("file_transfer_template_test: error: {e:?}").as_str());
